...
const CartModal: React.FC<CartModalProps> = ({ closeModal }) => {
    ...
    useEffect(() => {
        const fetchCartItems = async () => {
            try {
                const response = await axios.get('http://54.146.118.222:8000/cart/items/');
    ...
    const handleQuantityChange = async (productId: number, newQuantity: number) => {
        try {
            ...
            await axios.put(`http://54.146.118.222:8000/cart/items/update/${productId}/`, { quantity: newQuantity });
            // Rest of the code...
        } catch (error) {
    ...
    const handleRemoveItem = async (productId: number) => {
        try {
            await axios.delete(`http://54.146.118.222:8000/cart/item/remove/${productId}/`);
                    ...
                    {products.length === 0 ? (
                        ...
                        {products.map(item => (
                            <div className={classes.cartModal__product} key={item.id}>
                                <img src={`http://54.146.118.222:8000${item.product.image}`} alt="" className={classes.cartModal__productImage} />
